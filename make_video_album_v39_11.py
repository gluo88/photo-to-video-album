...
v36     use ffmpeg only: zoompan + drawtext + boxblur  
        having major performance enhancement. 8-20 time faster.  i
        Processed 1480 photoe (200 per chunk) in about one hour
v39.11  Fixed video frame rate, Added -r {fps} to process_video to prevent duration stretching
  

FEATURES: Ken Burns (by ffmpeg only), title/subtitble/captions,Chinese fonts, 
        photo filename overlay option(2.5 slower when being enabled)
        two types of yaml files for comfiguration

Note to LMM (please do not remove this note): 
     1. please update this revision history by adding new content and keeping all existing contents.
"""


"""
Cinematic Video Album Generator v39.11 – Fixed video frame rate
- Added -r {fps} to process_video to prevent duration stretching
- All other features unchanged (photo filename overlay, Chinese fonts, etc.)
"""
...
def build_audio_track_ffmpeg(music_files, total_duration, output_path, crossfade=2):
    ...
    for f in itertools.cycle(music_files):
        if total_dur >= total_duration:
            break
        loop_files.append(f)
        total_dur += get_media_duration(f)
    concat_list = os.path.join(os.path.dirname(output_path), "audio_concat.txt")
    with open(concat_list, "w") as f:
        for mf in loop_files:
            f.write(f"file '{mf}'\n")
    temp_concat = output_path.replace(".m4a", "_temp.m4a")
    cmd_concat = [
    ...
    run_ffmpeg(cmd_concat, "concatenating audio")
    cmd_trim = [
    ...
    run_ffmpeg(cmd_trim, "trimming audio")
